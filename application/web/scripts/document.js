...
const stringifyMap = (map) => {
    ...
    for (let i = 0; i < keys.length; i++) {
        text.push(`{ \"${keys[i]}\": \"${JSON.stringify(map[keys[i]])}\"`)   
    }
    return text.join("\n")
}
...
const BUILT_IN_EXPORTS = [
    {
        ...
        objects: "Button, Card, Container, Divider, Dropdown, Grid, Form, Header, Icon, Image, Menu, Message, Modal, Label, List, Popup, PopupHeader, Segment, Search, TextArea"
    },
    {
        path: "react-router-dom",
        objects: "BrowserRouter, Link, Route, Switch, useParams"
    },
    {
        path: "react",
    ...
    {
        path: "react-router",
        objects: "Redirect, useLocation, useHistory"
    },
    {
        path: "joi",
        objects: "Joi\'"
...
const getImportedObjects = (filePath, exportedFieldMap, lines) => {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (importPresent) {
            let importLine = line
            let j = i
            if (line.includes("App.css") || line.includes("/resources")) {
                continue
            }
...
const clearImportLines = (fileLines, importedObjects) => {
    console.log(stringifyList(importedObjects))
...
const updateImportedObjects = (importedObjects, exportedObjects, exportedFieldMap, filePath, fileText, clearedText) => {
    ...
    const updateExistingImportedObjects = (importedObjects, exportedFieldMap, filePath) => {
        let dirty = false
        const alreadyImportedList = []
        for (let i = 0; i < importedObjects.length; i++) {
            const importedField = importedObjects[i]
            const importedName = importedField.name
            if (alreadyImportedList.includes(importedName) || !(importedName in exportedFieldMap) || !fileTextIncludes(clearedText, importedName) || importedField.path == "./") {
                dirty = true
                importedField.toBeDeleted = true
                alreadyImportedList.push(importedName)
                continue
            }
            alreadyImportedList.push(importedName)
            const exportedField = exportedFieldMap[importedName]
...
const rewriteImportedObjects = (importedObjects, exportedFieldMap, filePath, fileLines, write) => {
    ...
    */
    const documentLines = (lines) => {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            const isExportConst = subEquals(line, "export const ")
            const isConst = subEquals(line, "const ")
            if (i > 0 && (isExportConst || isConst)) {
                const previousLine = lines[i - 1].trim()
                if (i > 0 && subEquals(previousLine, "*/")) {
                    freshFromImports = false
                    freeLineCount = 0
                    newLines.push(line)
                    continue
                }
                const name = line.substring((isExportConst ? 10: 3)).split(" ")[1]
                let min = (freshFromImports ? 1: 3)
                let difference = min - freeLineCount
    ...
    for (let i = 0; i < pathList.length; i++) {
        ...
        if (!path.includes("/") || path.includes("MobileContainer")) {
            continue
        }
        const adjustedPath = "../" + path
        const fileText = require(adjustedPath)
        const lines = fileText.split("\n")
        /*fs.writeFile("./scripts/backups/" + JSON.stringify(Date.now()) + ".js", lines.join("\n"), (err) => {
            if (err) {
                throw err
            }
        })*/
        const newFileText = documentLines(lines)
        const write = true
        console.log("START: newFileText")
        console.log(newFileText)
        console.log("FINISH: newFileText")
        if (write) {
            fs.writeFile("./" + path, newFileText, (err) => {
                if (err) {
                    throw err
                }
            })/**/
