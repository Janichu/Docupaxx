...
const BUILT_IN_EXPORTS = [
    {
        ...
        objects: "Button, Card, Container, Divider, Dropdown, Grid, Form, Header, Icon, Image, Input, Menu, Message, Modal, Label, List, Popup, PopupHeader, Segment, Search, TextArea"
    },
    {
...
const adjustAllImports = () => {
    ...
    */
    ...
    for (let i = 0; i < pathList.length; i++) {
        ...
        if (!path.includes("/") || path.includes("MobileContainer")) {// || !path.includes("GeneralDocumentsTab")) {
            continue
        }
        const adjustedPath = "../" + path
        const fileText = require(adjustedPath)
        const lines = fileText.split("\n")
        const importedObjects = getImportedObjects(path, exportedFieldMap, lines)
        const clearedLines = clearImportLines(lines, importedObjects)
        const clearedText = clearedLines.join("\n")
        const dirty = updateImportedObjects(importedObjects, exportedObjects, exportedFieldMap, path, fileText, clearedText)
        if (dirty) {
            rewriteImportedObjects(importedObjects, exportedFieldMap, path, lines, true)
        }
    }
